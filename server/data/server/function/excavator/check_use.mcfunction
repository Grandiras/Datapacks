# check if the excavator was used
execute unless score @s mined_with_excavator matches 1.. run return fail

# check if it was actually a excavator and not e.g. a netherite shovel
...
execute as @e[type=armor_stand,tag=last_block_mined] if score @s player_id = excavator_current player_id as @s at @s positioned ~ ~ ~ run function server:excavator/mine_chunk
